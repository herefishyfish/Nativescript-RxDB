...
function WebSqlPouch(opts, callback) {
  ...
  function runMigration2(tx, callback) {
    ...
    dbExecuteSql('ALTER TABLE ' + BY_SEQ_STORE +
      ' ADD COLUMN deleted TINYINT(1) DEFAULT 0', [], function () {
      dbExecuteSql(BY_SEQ_STORE_DELETED_INDEX_SQL);
      dbExecuteSql('ALTER TABLE ' + DOC_STORE +
        ' ADD COLUMN local TINYINT(1) DEFAULT 0', [], function () {
        dbExecuteSql('CREATE INDEX IF NOT EXISTS \'doc-store-local-idx\' ON ' +
          DOC_STORE + ' (local, id)');
        ...
        dbExecuteSql(sql, [], function (tx, result) {
          ...
          dbExecuteSql('UPDATE ' + DOC_STORE + 'SET local = 1 WHERE id IN ' +
            qMarks(local.length), local, function () {
            dbExecuteSql('UPDATE ' + BY_SEQ_STORE +
    ...
    dbExecuteSql(local, [], function () {
      ...
      dbExecuteSql(sql, [], function (tx, res) {
        ...
        function doNext() {
          ...
          dbExecuteSql('INSERT INTO ' + LOCAL_STORE +
              ' (id, rev, json) VALUES (?,?,?)',
              [row.id, rev, row.data], function (tx) {
            dbExecuteSql('DELETE FROM ' + DOC_STORE + ' WHERE id=?',
                [row.id], function (tx) {
              dbExecuteSql('DELETE FROM ' + BY_SEQ_STORE + ' WHERE seq=?',
                  [row.seq], function () {
                doNext();
              });
            });
    ...
    if (dbVersion === 0) {
      ...
      var setupDone = function () {
        ...
        if (migrated) {
          ...
          db.execute('UPDATE ' + META_STORE + ' SET db_version = ' +
            ADAPTER_VERSION);
        }
        ...
        db.select(sql, sqlArgs).then((result) => {
          ...
          for (var i = 0, l = result.length; i < l; i++) {
            ...
            if (filtered) {
              ...
              if (opts.attachments && opts.include_docs) {
                fetchAttachmentsIfNecessary(doc, opts, api, db,
                  reportChange(change));
              } else {
                reportChange(change)();
              }
